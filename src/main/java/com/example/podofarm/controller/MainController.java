...
public class MainController {
    ...
    public String studyMainPage(Model model, @PathVariable("s_code") String s_code, HttpSession session) throws JsonProcessingException {
        ...
        System.out.println("세션값 확인: id는 " + id + " 스터디 코드는 " + s_code);
        ...
        // 2차원 배열 선언
        String[][] solvedDataTypeStrings = new String[index][DayCheck];

        // 멤버마다 푼 문제를 확인
        for (int i = 0; i < index; i++) {
            ArrayList<Map<String, String>> solvedList = codeService.getSolvedByDayCurrentMonth(memberID.get(i));
            ...
            for (Map<String, String> map : solvedList) {
                ...
                solvedMonth[day - 1] += Integer.parseInt(solved);
            }

            for (int j = 0; j < DayCheck; j++) {
                switch (solvedMonth[j]) {
                    case 0:
                        solvedDataTypeStrings[i][j] = "solved-0";
                        break;
                    case 1:
                        solvedDataTypeStrings[i][j] = "solved-1";
                        break;
                    case 2:
                        solvedDataTypeStrings[i][j] = "solved-2";
                        break;
                    case 3:
                        solvedDataTypeStrings[i][j] = "solved-3";
                        break;
                    default:
                        solvedDataTypeStrings[i][j] = "solved-4";
                        break;
                }
            }
        }

        // 2차원 배열의 각 요소를 모델에 추가하고 출력
        for (int k = 0; k < index; k++) {
            model.addAttribute("solvedData" + (k + 1), solvedDataTypeStrings[k]);
            System.out.println("solvedData 확인용 solvedData" + (k + 1) + ": " + Arrays.toString(solvedDataTypeStrings[k]));
        }

        model.addAttribute("memberSize", index);
